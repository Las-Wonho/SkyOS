...
두번째 줄에는 레지스터 초기화이다. EFLAGS 래지스터를 초기화 한다. EFLAGS레지스터는 알아서 찾아보면 되는데 딱히 설계까지 앍는 귀찮으니 책에 알아야 하는 내용만 쓰여있으니 참고하자.
...
위에 명령어의 의미는 [이 문서](http://www.cplusplus.com/reference/cstdarg/va_list/)보면 나와있다. 분명 찾기 귀찮아할테니 설명하자면, 가변 안자있잖아. >printf("%d, %d",1,2)< 같이 인자가 계속 있는 경우에는 저런 va_*같은 애들을 써서 처리를 해줘야해. C에서 쓰이는 애들이라는데 안써봐서 모른다. (자주써봤다는데 C를 깊게 배우지 않아서 미안) 그리고 아래에서 itoa_s도 쓰이는데 아스키 코드값 얻어오는 아이라고 한다.
...
```c++
	...
	case 8:         // 백스페이스
		...
		*(m_pVideoMemory + t) = ' ' | ((unsigned char)m_Color << 8);		
		break;
```
여기서 잘 보면 포인터를 사용하서 문자를 지우거나 출력한다. 즉, 어셈블리처럼 특정 장소에 바이트를 잘 넣기만 하면 콘솔출력이 가능하다.